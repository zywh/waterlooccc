...
# CCC 2015 Junior Problem 5: Pie Day
# Dynamic Programming / Memoization solution

import sys
sys.setrecursionlimit(10000)  # in case recursion depth is high

# Create a 3D DP dictionary to store results
visited = {}

def pi(n, k, minimum):
    """
    n: number of remaining pieces of pie
    k: number of remaining people
    minimum: minimum pieces the next person can take
    """
    # Base cases
    print(n, k, minimum)
    if k == 1 or n == k:
    ...
    # Check memoization
    if (n, k, minimum) in visited:
        return visited[(n, k, minimum)]
    
    total = 0
    max_take = n // k  # maximum first person can take
    
    for i in range(minimum, max_take + 1):
        total += pi(n - i, k - 1, i)
    
    # Store in memoization dictionary
    visited[(n, k, minimum)] = total
    return total
...
print("Number of ways to distribute pie:", pi(n, k, minimum))
